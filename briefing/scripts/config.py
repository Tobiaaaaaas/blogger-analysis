...
v14（2026-09-04）：展示窗口由自然日改为**交易日口径**（用户修正）：
  - 超短板块（17）：扫 WINDOW_TRADING_DAYS["short"]=1 → 前一交易日 00:00 起至现在的
    ...
    （TRADING_TICKS 10 档）。
  - 波段板块（30）：扫 WINDOW_TRADING_DAYS["swing"]=3 → 前 3 个交易日 00:00 起至现在的
...
名单口径（2026-09-06 更新）：
  - 超短板块（17）：来自 reports/top20_值得关注博主.md 超短榜（顺序即展示顺序，未变）。
  - 波段板块（30）：波段档（信号日→验证终点≥2 交易日）三条件全严格达标者 28 人（均分>0.2
    且 夏普>0.1 且 胜率>50%，数据截止 2026-09-04）按均分降序 + 原委员会保留 刘海娃娃/顺应周期
    殿后；剔除 红红火火的老牛哥、微风3241。完整筛选口径见 reports/comparison_direction.md 与
    当期《波段达标名单》。顺序即板块内展示顺序。
"""
