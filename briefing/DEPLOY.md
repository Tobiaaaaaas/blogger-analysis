...
- **超短卡** → 旧群（env `FEISHU_WEBHOOK_URL`）：交易日盘中**每 30 分钟一档**（09:30/10:00/10:30/11:00/11:30/13:00/13:30/14:00/14:30/15:00），扫近 2 自然日。
- **波段卡** → 新群（env `FEISHU_WEBHOOK_URL_SWING`）：**三档**（09:30/11:00/14:30），扫近 5 自然日。
...
- **窗口口径（用户确认接受）**：超短近 2 自然日 → 周一早晨不含上周五"看周一"的帖；波段近 5 自然日同理边界。需要覆盖更远历史放宽 `WINDOW_CAL_DAYS` 即可。
