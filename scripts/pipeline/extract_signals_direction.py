...
## 提取为信号的标准（可打分性）
必须满足：**明确的态度**（看涨/看跌、收阳/收阴、涨/跌）。有明确预测周期最好；**没有明确预测周期但有明确方向，也提取**（标 spec=nd，由引擎按未来 5 个交易日收益率单点计分，独立成档）。
...
- **无明确预测周期但有明确方向**（"大趋势向上""随时会涨""肯定要涨但不知道什么时候"）→ spec=nd，cat=scored（引擎按未来 5 个交易日收益率单点计分，独立成档，不参与总榜/多空/三档）
