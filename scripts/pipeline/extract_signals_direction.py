...
SPEC_RE = re.compile(r"^(today|week|nweek|nweek_first|month|nmonth|long|nd|t\d+|d:\d{4}-\d{2}-\d{2})$")
...
## 提取为信号的标准（可打分性）
必须满足：**明确的态度**（看涨/看跌、收阳/收阴、涨/跌）。有明确预测周期最好；**没有明确预测周期但有明确方向，也提取**（标 spec=nd，由引擎按 3/5/10 日收益率加权计分，独立成档）。
...
- **无明确预测周期但有明确方向**（"大趋势向上""随时会涨""肯定要涨但不知道什么时候"）→ spec=nd，cat=scored（引擎按 3/5/10 日收益率加权计分，独立成档，不参与总榜/多空/三档）
...
4. **补加**：原文有比已提取信号**更明确或被漏掉**的预测结论 → 放入 add。典型遗漏：① 同一帖子里还有**第二个观点**（不同方向或不同周期，如"短期看空、长期看多"只提取了短期看空 → add 长期看多那条：long/unscored）；② 应属 nd（无周期有方向）或 long/unscored 而未被提取。
